...
**Verified against:** commit `e63d86dc` on `phase-2-adapter-wiring` (JARVIS-Automation)

> **What changed in v3.19.** **PLATFORM_UNKNOWN honest-failure gap (flagged, not fixed, in v3.14's finding #2) now fixed.** `IntentRouter.classify()` silently fell through PLATFORM_UNKNOWN text to the generic ACTION_PATTERNS match, landing in `ODAVLoop`'s OBSERVE/DECIDE/ACT pipeline, whose `open_app` fallback shells out via `subprocess.Popen(target, shell=True)` with no honest check -- Windows' own cryptic error then surfaced almost verbatim (reproduced live with "open warfare"). `classify()` now routes open/close-a-name text under PLATFORM_UNKNOWN to a new `action_platform_unknown` handler, the same honest-failure shape as `action_no_adapter`/`action_not_installed`. Deliberately scoped to open/close only, not the full open/close/launch/start/run verb set named in the original instruction: "run"/"start" collide with real, higher-priority phrasings checked later in `classify()` (`CODE_PATTERNS`' "run tests"/"run checks", `RHINAL_START_CASE_PATTERNS`' "start a new case: ..."), both confirmed misrouted during this fix's own test run before being narrowed -- kept exactly as wide as the reported gap, not wider; "launch"/"start" outside those specific collisions still fall through to the pre-existing generic path, unchanged. Commit `0e12da82`, 7 new tests including an explicit non-regression pin for the collisions found. **Push required two silent retries** -- GitHub's API returned a transient `Internal Server Error` on the first two attempts (`git push` itself, not a local or code issue); the third attempt succeeded and the remote was verified to match local `HEAD` exactly before this closure was reported.
>
> **What changed in v3.18.** **Wake-thread concurrency defect (flagged, not fixed, in v3.14) now fixed.** Root cause was already correctly traced in v3.14 — `_on_wake_detected()` ran the full wake-handling chain synchronously on `WakeDetector`'s own listening thread, so `_return_to_sleep()`'s later call to restart listening could spawn a second `_listen_loop` thread before the first had unwound and closed its mic stream. Fixed by decoupling: `WakeDetector`'s callback is now `_signal_wake_detected()` (sets a `threading.Event`, nothing else — safe on the listening thread), and the real handling (`_handle_wake_detected`, including the stop/restart cycle) now runs from `PersistentWakeService.start()`'s own main-thread loop. `WakeDetector.stop()` now genuinely blocks (`Thread.join(timeout=5.0)`) instead of returning immediately, and raises `RuntimeError` if ever called from the listening thread's own call stack — the exact shape of the original bug, now structurally caught rather than just fixed once. Commit `eb13ebd9`, 8 new tests. `WakeService/jarvis_service.py`'s separate legacy entry point still has the old synchronous pattern and is deliberately unfixed, matching existing precedent already noted in `wake_detector.py`'s own comments. **Model note:** instructed as an Opus 5 item (threading redesign, safety/architecture-adjacent); executed as Sonnet 5 within this session instead — stated plainly here rather than silently substituted, consistent with this project's standing practice of naming deviations rather than absorbing them quietly.
>
> **What changed in v3.17.** **Orb wiring completed — v3.16's "does not exist" finding was correct at the time it was checked, and is now superseded by a real file, not contradicted.** `jarvis_orb.py` existed outside this repository; the project owner located and provided the actual 197-line source directly, unmodified. Committed at `c4ad42cf` alongside `tests/test_jarvis_orb.py` (9 tests). Wired exactly as originally specified: a single static frame (`render_frame(0.6, 0.3)`) printed above the compact status box in `PersistentWakeService.start()` on every launch — the animated `play()` intro stays reserved for first-run onboarding only, not wired here. Verified live, not just under unit test: a real `python jarvis.py --service` launch was captured, showing the orb's actual ASCII output printing above the real status box. §1.7's line is corrected in place below.
>
> **What changed in v3.16.** **Orb wiring instruction declined, not attempted — a hard stop, reported rather than routed around.** Instructed to wire `jarvis_orb.py`'s existing static-frame render into the boot sequence, on the stated premise that the module "already exists and was tested." Checked before writing anything: `jarvis_orb.py` does not exist anywhere — not in this repo on any branch (`git log --all` for the filename returns nothing), not in any sibling checkout on this machine, not in `Downloads/` or `OneDrive/`. §1.7's own line asserting it as "built and approved" is corrected in place rather than left standing uncontradicted next to this finding. Building the module now would be new design work passed off as wiring, which the instruction itself explicitly excluded ("no new design work... purely a wiring gap") — so nothing was built. Full detail and what would actually need deciding first, in the execution log.
>
> **What changed in v3.15.** **`WakeService/local_stt.py` swapped from `vosk-model-small-en-us-0.15` to `vosk-model-en-us-0.22`**, measured before and after rather than assumed. On the two exact phrases reported failing live, the small model mis-transcribed "open spotify" as "open spot if i" and "open whatsapp" as "open what's app" (neither matches any adapter alias); the large model got both exactly right. Real cost, also measured rather than assumed: 2.7GB on disk (was 68MB) and **35-95s to load, varying across repeated runs on identical hardware** — the variance itself is the finding, not just the size. `WakeService/wake_detector.py`'s grammar-restricted wake-word detector deliberately stays on the small model — a different, continuously-running, latency-sensitive task the reported failures never implicated. A real test regression was found and fixed in the process (`tests/test_local_stt_no_cloud_path.py`'s hardcoded 30s subprocess timeout, now stubbing Vosk entirely since that test never needed a real model load to begin with). Not escalated to Whisper — the gap the larger Vosk model was asked to close, closed, on the reported phrases; `vosk-model-en-us-0.22-lgraph` (128MB) is named as the next thing to measure if the load-time cost proves unacceptable, not decided here. Full suite: 698 passed, 1 skipped, 2 failed — both a pre-existing, already-logged `pyautogui` mouse-position flake from DEC-002, confirmed unrelated via `git log` on every file involved.
>
> **What changed in v3.14.** **Live smoke test** (explicit instruction, not a build phase): a real manifest of what's wired (not this document's aspirational list), a real session against it, bounded fix authority for failures matching a class already fixed this session. Found and fixed two genuine bugs, both live-reproduced via voice: `AgentCore/odav_loop.py`'s DECIDE/ACT/VERIFY pipeline read `step.action` on `ActionStep`/`ExecutionStep` (only `.action_type` exists) and `result.success` on `UIExecutor.execute_step()`'s return (only `.ok` exists) — either one crashed *any* action command reaching the generic OBSERVE/DECIDE/ACT pipeline (not the adapter-resolved fast path), reproduced by "open warfare." `WakeService/wake_detector.py` fired the wake callback on every Vosk partial result stabilizing to "jarvis" with no debounce — reported live as one utterance activating two agents — fixed with a 1.5s cooldown. **Two findings surfaced and deliberately left unfixed, per this phase's own hard-stop instruction:** a deeper wake-thread concurrency defect (`_on_wake_detected()` runs synchronously on `WakeDetector`'s own listening thread, so `_return_to_sleep()` can restart listening via a second thread before the original has unwound and closed its stream — a genuine architecture decision, not a mechanical fix); and `PLATFORM_UNKNOWN` commands (matching no known adapter at all) bypassing `ResolutionGate`'s honest-failure design entirely, falling through to a raw subprocess call that surfaces Windows' own cryptic error instead of a physician-facing message — never previously decided whether this branch should get the same honest treatment as `NOT_INSTALLED`/`NO_ADAPTER`. Also live-confirmed as part of the manifest: RHINAL's honest `RhinalConfigError` fires correctly with no credentials configured, and that same call produced **the first real `data/audit/clinical_actions.log` this repo has ever had outside a test tmp dir** — proof the DEC-002 audit chokepoint genuinely emits, not just passes tests. Full trail, commit `e63d86dc`, in the execution log. **Stage 0.5 remains formally BLOCKED, unaffected — a clean smoke test is not Stage 0.5 validation; the real gate (physician access in real clinic conditions, a pre-registered Stage 1 priority list committed first) is unchanged.**
...
>
> **What changed in v3.7.** **Resolves D15 by splitting it**: `code_engine` is owner-confirmed intentional (build infrastructure, same category as Level6, never meant for the clinical action path); `rhinal_capture`'s unaudited external vault write is the real, narrower gap, split out as **D16** and fixed in its own phase. **Resolves the Boundary Ledger's scope ambiguity** — §Stage 0.5 read as "deterministic rule was wrong" (JARVIS-internal) while the v3.0 reframe read as "hospital software fails to complete an action" (hospital-wide); they were never two systems. Now stated consistently as **capture broad, classify downstream**: the ledger is a flight recorder that logs every observed completion failure undifferentiated at capture time, and Adaptive Adapter Generation's trigger logic separates *fixable by us* from *not fixable by definition* at analysis time. Both classes stay in the ledger permanently; only one routes to code generation, and the not-fixable class is the Stage 2 COO buyer's evidence rather than noise. **D16 fixed in the same run** (commit `64055a1b`): `AgentCore/mcp_audit.py`'s `audited_mcp_write()` covers `rhinal_capture`'s external vault write, reusably for the other 13 RHINAL tools, emitting two records per write (`attempted` before the wire, outcome after) because the client raises one exception type for both "unreachable" and "server rejected" — so a failure return cannot prove nothing was written remotely. Adds the **model benchmark scanner** (`AgentCore/model_scanner/`) informing §4.4b's developer-configured model slot — advisory only, classified `general` explicitly before implementation per §3.7b, dual staleness timestamps. Adds **D17**, a pre-existing rescanner test that cannot pass when `_installed_map()` is slow (~0.97s per tick even with a `Mock` against a 0.45s budget for ≥2 iterations) — proven pre-existing on a clean `HEAD` worktree, logged not fixed.
>
> **What changed in v3.8.** **Companion-app Phase 0 — three safety decisions resolved before any app code**, because the app's safety model depends on all three. **(1) PG-001's Mode C drift-catch detector is built** (`AgentCore/privacy_gateway/`), local rules/lexicon, confidence-scored, with full Detector Assurance measured on a committed synthetic corpus and a CI check that fails if any published number is absent from a fresh run. The numbers are deliberately unflattering — **65% drift recall (35% false-negative rate), and 0 of 8 colloquial clinical utterances caught** — which is the measured proof of why PG-001 already refuses to let Mode C's safety property rest on a classifier. **(2) Trust-Based Authorization split into four tiers** ordered by irreversibility and liability; **T3 clinical write** gets per-action confirmation always, sits above bank transfer deliberately, and a channel that cannot meet its four invariant properties refuses rather than downgrades — with principle 4 gaining a carve-out for the contradiction that created. **(3) The sandbox package-access question, logged unresolved since v3.3, is resolved** as pre-staged-not-proxied with a machine-readable vetted manifest and an expiring escalation path. Adds **D18** — the Level6 "sandbox" is a subprocess against host site-packages, and its `JARVIS_SANDBOX_NETWORK=0` is written in one place and read in none; §4.4b's gating table is therefore a specification for an unbuilt image, not a description of today. **Stage 0.5 remains BLOCKED and unaffected** — the companion-app track is parallel, synthetic-data-only, and does not move Stage 0.5's gate.
>
> **What changed in v3.9.** **D18 closed with real, independently-verified isolation.** The Level6 sandbox no longer runs the host interpreter against host site-packages: generated code runs in a provisioned standalone interpreter inside an AppContainer with no capabilities, a restricted token, and a job object — all non-admin, because Docker/WSL/Windows Sandbox are all absent on this machine and admin is not held (each checked before designing). `JARVIS_SANDBOX_NETWORK` deleted rather than repaired, with an AST test blocking its return. §4.4b's Clinical-data, Secrets and Network-egress rows move from specification to **enforced and verified** — a reviewer-authored probe with a control arm confirms a canary in the user's home directory, home-directory enumeration, outbound TCP and DNS all fail contained and all succeed uncontained on the identical script and interpreter. Adds **D19**: the sandbox has **no supply-chain integrity** — the closure is copied from host site-packages with no hashes or pinning — split out deliberately so D18's closure does not leave a manifest promising hash-pinning the code does not do. **Stage 0.5 remains formally BLOCKED** on two unmet prerequisites (physician access in real clinic conditions; a pre-registered Stage 1 priority list committed *before* that access). No implementation work was done against it, and E2 is explicitly not redefined down to what is reachable without a physician.
>
> **What changed in v3.10.** **Companion-app Phase 1 scaffolding built** (`AgentCore/comm_gateway/`, design note `docs/companion_app_scaffold.md`), gated on Phase 0 and D18 landing first. Project structure, the auth handshake, and the Session-creation flow — not feature-complete, Track A only. Session carries no channel field at all (checked structurally via `dataclasses.fields()`, one level down into CapabilitySet too), enforcing "Session Manager must never know which channel a session originated from." STRICT_LOCAL is the enforced default in two independent places, so code that forgets to load a policy still gets the safe value. No channel reaches T3 in this phase for any deployment mode — not because desktop voice is architecturally excluded (the trust-tier doc names it as an intended T3 channel) but because the four-invariant confirmation mechanism isn't built for anything yet. "Never renegotiated upward mid-session" is enforced by `SessionManager.register()` refusing re-registration of an existing session_id, not only by `CapabilitySet` being frozen — a frozen dataclass stops one instance being mutated, not a stronger one being substituted in. §4.4c's "all four items not started" is corrected: three of four now have scaffolding; JVMA remains genuinely unstarted.
>
> **What changed in v3.11.** **D18 re-verified unchanged** (three-layer boundary and the deleted `JARVIS_SANDBOX_NETWORK` read directly from the current code, not assumed from v3.9's closure). **D19 partially closed.** Re-examined D19's original "blocked on infrastructure" framing before acting on it, per instruction, and found it conflated two different things: the manifest's container-image hash-pinning (`pip install --require-hashes` against a package index) genuinely needs a container runtime this machine doesn't have and stays PENDING; but hash-pinning the files `sandbox_env.py` actually copies from the host is a checksum problem, not an infrastructure one. Built: `AgentCore/policy/adapter_sandbox_provisioned.lock.json`, a reviewed baseline of per-file hashes over the full resolved closure, verified by `provision()` before anything is built or copied, on every call including cache reuse, refusing with `SupplyChainIntegrityError` on any mismatch. Verified against a REAL host file, not a simulated one: a `packaging` dist-info file was genuinely mutated on disk, `provision()` refused, the file was restored, and the restoration was itself checked (`test_supply_chain_lock.py`). This closes the drift/tamper half of D19; the provenance half (verifying host packages were ever legitimate) remains open and stays with the container-image mechanism. **The scoped loopback exception was investigated, not delivered.** `LoopbackExceptionPipe` ACE-grants a single named pipe to the AppContainer SID — real, working, and adversarially verified for AppContainer running alone, after fixing four genuine bugs found while building it. Against `ContainedLauncher`'s actual default configuration (AppContainer + restricted token), the same grant is still refused; isolated by control to the restricted token specifically (job object is not a factor). Root cause not fully isolated beyond that. Kept in the codebase with both the positive and negative results pinned as adversarial tests, rather than removed or left claiming to work. **Container-upgrade-path note added** (`docs/adapter_sandbox_isolation.md` §9): if Docker/WSL/Administrator ever become available, D18's three-layer stack, the D19 host-baseline lock, and `LoopbackExceptionPipe` are all replaced by real container isolation, not layered under it. **Stage 0.5 remains formally BLOCKED**, untouched this phase.
>
> **What changed in v3.13.** **RHINAL 13-tool wiring: 12 of 13 wired** (`rhinal_attach_file` stays untouched, a named hard stop). Risk classes independently re-confirmed by reading `mcp-server/src/{index,tools}.ts` directly rather than trusted from the earlier verification entry's summary — 7 read-only (`recall`, `ask_vault`, `classify_worthiness`, `confront`, `get_calibration_score`, `get_case_graph`, `check_contradiction`), 5 write-capable (`decision_log`, `idea_to_spec`, `tag_prediction`, `resolve_prediction`, `start_case`). A real routing collision was found and fixed: the existing capture pattern for "log this decision"/"log that idea" was narrowed and the two dedicated tools took over that phrasing, correctly, now that they exist. A real dispatch bug was found by the new tests before anything shipped: the client was being constructed even for empty-input clarification prompts that should be genuine no-ops. Each write-capable tool individually verified with a real before/after mutation check against the live backend — not just "the call succeeded" — matching D19's mutate-and-confirm-refusal standard: case count +1 with the right title, the saved record's own `mode` field matching what was requested, calibration score fields moving by exactly the expected amount. No real patient/clinical data used anywhere — all test writes clearly labeled or the agent's own prior test records. Full suite: 692 passed, 1 skipped, 0 failures.
>
> **What changed in v3.12.** **D17 closed.** Investigated from the current code rather than the earlier finding's own description of it — confirmed the real cause: `CommandRouter._default_adapter_classes()` rebuilt all 14 default adapters from scratch on every unparameterized `CommandRouter()`, discarding 13 of the 14 adapters' worth of work just to read off types, with no caching of the (static, process-lifetime-fixed) result. Fixed with a module-level cache. Answers the deferred "why does a background tick cost ~1s" question the original finding raised but didn't chase: it didn't need to cost anything, it was just never memoized. Test also converted from a fixed-sleep assertion to a deadline poll, since a warm cache doesn't guarantee an isolated test's own first `CommandRouter()` is free. Verified 3/3 in fresh processes. Full suite unchanged at 630 passed, 1 skipped, 0 failures.
...
| D7 | `ODAVLoop.execute()` incomplete gate-outcome handling. **Corrected v3.14: "unreachable from live loop today" was stale/wrong** — live-reproduced via voice ("open warfare" going through `PersistentWakeService` → `self._odav.execute()`), which is exactly how the v3.14 smoke test found and fixed two crash-causing attribute mismatches in this method (see execution log). Reachable whenever a command doesn't resolve to a known adapter via `ResolutionGate` and falls through to the generic `ACTION_PATTERNS`/OBSERVE-DECIDE-ACT path. | 🟡 Low |
...
| ~~**D18**~~ | ~~The Level6 "sandbox" is not a sandbox, and one of its variables reads as a control it does not implement.~~ — **✅ RESOLVED**, commit pinned in the execution log. `sandbox_runner.py` no longer runs the host interpreter against host site-packages. Generated code now runs in a **provisioned standalone interpreter** (own `sys.prefix`, populated only from the vetted manifest) inside **three stacked OS-enforced boundaries**, all non-admin: an **AppContainer with no capabilities** (WFP denies every socket; LowBox token denies files lacking an ACE for the package SID), a **restricted token** with the user SID deny-only (the user profile's own ACLs stop applying), and a **job object** (kill-on-job-close, process/memory/CPU caps, UI restrictions). The launcher **refuses to run if a required layer is unavailable** rather than degrading silently — the degradation being the original defect. `JARVIS_SANDBOX_NETWORK` is **deleted, not repaired**, and an AST-based test fails if it is reintroduced. **Independently verified by the reviewer, not accepted from the implementing pass:** a parent-authored probe ran the identical script and identical interpreter twice, contained and uncontained — contained, a canary in the user's home is `PermissionError`, `~` enumeration is `PermissionError`, TCP to 1.1.1.1:443 is `PermissionError [WinError 10013]`, DNS is `gaierror`; uncontained, the same probe read the canary, listed 99 entries, connected, and resolved. The control arm is what makes this evidence rather than a passing assertion. **Container isolation was not available** (no Docker, no WSL, no Windows Sandbox, not admin — all four checked), so this is what the machine genuinely supports, and it is not a container: same kernel, same filesystem, same network stack. | ~~🟠 High~~ closed |
| ~~**D17**~~ | ~~`test_calls_refresh_repeatedly_on_interval` cannot pass on a machine where `_installed_map()` is slow.~~ — **✅ RESOLVED, v3.12.** Root cause confirmed by direct measurement, not the earlier finding's assumption: `CommandRouter._default_adapter_classes()` reconstructed all 14 real adapter instances from scratch on every unparameterized `CommandRouter()`, just to read off their types — nothing cached the class map itself. Fixed with a module-level cache (`AgentCore/command_router.py`); repeated construction measured at 0.0s per call afterward, down from ~0.28-0.6s. Answers the "more interesting question" the original finding named: a background tick cost ~1s because it recomputed a static mapping every time, not because the work was inherently expensive. Test also made deadline-based (`test_onboarding.py`) rather than fixed-sleep, since caching alone doesn't guarantee the rescanner's *first* tick in an isolated process is fast. Verified 3/3 in fresh processes, matching the original reproduction. Full suite unchanged at 630 passed, 1 skipped, 0 failures. | ~~🟡 Low~~ closed |
| ~~**D19**~~ | **PARTIALLY RESOLVED, v3.11.** The original framing — "genuinely blocked on infrastructure, not effort" — was re-examined before being trusted, per instruction, and found to conflate two different mechanisms. The manifest's container-image hash-pinning (`image.base_digest`, `image.lockfile`, `pip install --require-hashes` against a package index) genuinely needs a container runtime this machine does not have; that half is **still blocked, unchanged**, and `adapter_sandbox_dependencies.yaml`'s `image:` fields stay null. But hashing the files `sandbox_env.py` actually copies from the host's site-packages needs nothing but a checksum, and that half is **now built and enforced**: `AgentCore/policy/adapter_sandbox_provisioned.lock.json` is a reviewed baseline of per-file hashes over the full resolved closure; `provision()` verifies the current host state against it before building or copying anything, on every call including cache reuse, and raises `SupplyChainIntegrityError` on any mismatch. **Verified against a REAL host file**, not a simulated one: a real `packaging` dist-info file was mutated on disk, `provision()` genuinely refused, the file was restored, and the restoration was checked too (`AgentCore/level6/tests/test_supply_chain_lock.py`, `TestProvisionRefusesOnRealHostTamper`). **What this does not close:** provenance. The lock pins to a reviewed baseline and catches drift/tampering since that baseline was committed; it cannot tell you the host's packages were ever legitimate to begin with — that remains the container image's job, and remains blocked on the same missing infrastructure as before. | 🟡 Medium — drift/tamper half closed; provenance half remains 🟠 High, blocked on infrastructure |
...
- `jarvis_orb.py` — ~~built and **approved**, untested on Windows terminal, not yet wired~~ ~~does not exist anywhere in the repository (v3.16)~~. **Corrected again, v3.17:** the v3.16 finding was accurate as checked — the file was genuinely absent from this repo, every sibling checkout, and common outside-git locations at that time. It existed outside the repository the whole time; the project owner located it and provided the real source directly. Now committed at `c4ad42cf`, wired into `PersistentWakeService.start()` (§3.8), live-verified, tested (`tests/test_jarvis_orb.py`, 9/9 passing). No longer dark capability — removed from future revisions of this list.
...
| **RHINAL MCP integration** | 13 of 14 tools WIRED (v3.13) | `rhinal_capture` (commit `80ae8cb`) plus 12 more: 7 read-only, 5 write-capable, each write-capable tool individually verified with a real before/after mutation check against the live backend (case count, `mode` field, calibration score deltas — not just "the call succeeded"). Risk classes independently re-confirmed from `tools.ts` source, not carried over from the earlier verification pass's summary. `rhinal_attach_file` remains the one unwired tool — still needs a real non-identifying test file plus explicit approval before it can be attempted; see execution log. |
...
2. **This threatens the obvious design.** "Restrict network egress in the sandbox" is exactly the pattern that failed. If adapter-generation needs package access to test generated code, that proxy is the attack surface — same pattern, same weakness. **Resolved — pre-staged, not proxied.** The sandbox image carries a fixed, vetted dependency set: base pinned by digest rather than tag, installed from a hashed lockfile covering the full transitive closure via `pip install --require-hashes`, no index configured, no `pip` on the runtime path. Live package fetching is not a default that can be switched off; it is absent. Anything outside the set is an explicit escalation reviewed once by the developer through Phase 3d's existing propose → approve → versioned commit idiom, recorded as a diff to the manifest — with `review_by` and `justified_by` fields CI enforces, so the set shrinks by default rather than accumulating. Design note and machine-readable manifest: `docs/adapter_sandbox_dependencies.md`, `AgentCore/policy/adapter_sandbox_dependencies.yaml`. **What this does not claim:** it removes the one route that was actually exploited, not the class. Khlaaf's "notoriously insecure" still stands; the ResolutionGate is still the primary control; a vetted package carrying its own CVE, a compromise already present at pin time (a hash pins what we got, not what was legitimate), and the container runtime itself all remain exposed, and are enumerated in the design note rather than summarised away here. **And the honest precondition:** there is no container today — `AgentCore/level6/sandbox_runner.py` is a subprocess running `sys.executable -m pytest` against the host's own site-packages, with an advisory `JARVIS_SANDBOX_NETWORK=0` that nothing reads (see **D18**). Until the image is built, the manifest is a review aid and the input to a static import check, not a boundary.
...
| Control | Requirement |
|---|---|
| Clinical data | Not in scope structurally — unavailable to send, not merely "we don't send it". **Now enforced, not asserted:** the child runs under a LowBox/AppContainer token with no capabilities and a restricted token denying the user SID, so the host profile is unreadable — independently verified, a canary in the user's home returns `PermissionError` inside and reads fine outside. |
...
| Secrets | No keyring access, no `.env`, no credential paths — **enforced by the OS, verified**: the user profile is unreadable inside (`PermissionError` on `~`, on `.gitconfig`, and on an arbitrary canary), and `keyring` and `cryptography` are absent from the provisioned interpreter entirely while present on the host. |
| Network egress | **Denied at the OS layer, verified** *(D18 closed; see finding 2)*. The child runs in an AppContainer with **no capabilities**, so Windows Filtering Platform refuses every socket: outbound TCP returns `PermissionError [WinError 10013]`, DNS returns `gaierror`, loopback times out — each confirmed against a control run of the identical script and interpreter that connects successfully. `JARVIS_SANDBOX_NETWORK` is **deleted**, not repaired; a test now fails if anyone reintroduces it. Dependencies are pre-staged from the vetted manifest with no index and no `pip` at runtime, and an unvetted import fails closed before execution. **Two things this is not:** it is not a capability restriction — `socket` and `subprocess` remain importable and only the *attempt* fails; and it is not full supply-chain integrity — the closure's provenance is unverified and the container-image hash-pinning remains blocked on infrastructure (see **D19**, though D19's drift/tamper half is now closed — v3.11). Loopback denial is a real functional cost: adapter tests needing a local server cannot run in-sandbox without an admin-granted exemption. A non-admin, single-pipe scoped exception was attempted and investigated (v3.11, `LoopbackExceptionPipe`) but does not work against the real launcher configuration — see D19's v3.11 note and `docs/adapter_sandbox_isolation.md` §7. |
...
*(Logged 2026-07-30, from a multi-turn design thread. All four items below are: developer-only with no public/patient-facing access, sequenced after DEC-002/audit-trail and RHINAL's remaining tools, and — per Ayan's explicit instruction — usable only against synthetic/test data until the deployment-policy question in each is actually settled. Registered as Ayan requested: whole entries, not pre-split by the reviewer's risk read — but each entry states its own internal tiering honestly so scale isn't hidden. **"Not started" no longer holds for three of the four, added v3.10:** Communication Gateway, PG-001's mode/capability logic, and Security Broker's approval-request shape all have Phase 1 scaffolding (`AgentCore/comm_gateway/`) — project structure, the auth handshake, and the Session-creation flow, not feature-complete implementation, still Track A only. JVMA remains genuinely not started. See the execution log's Companion-app Phase 1 entry.)*
...
4. **Channel Independence** — same local engine performs planning/execution regardless of which channel a command arrived through — **except T3 clinical writes, which some channels cannot carry at all; see principle 5.**
