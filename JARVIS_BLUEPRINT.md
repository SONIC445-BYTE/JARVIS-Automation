...
| **D19** | **The sandbox has no supply-chain integrity.** The vetted closure is *copied from the host's own site-packages* — no hashes, no pinning, no digest-pinned base, no verification that what was copied is what the manifest names. `AgentCore/policy/adapter_sandbox_dependencies.yaml` carries `hash: null` throughout, deliberately (nothing is built, and plausible-looking hashes would be worse than none) — but the consequence is that a host package already compromised at copy time propagates into every sandbox run unnoticed. **Logged separately rather than folded into D18's closure, on the implementing agent's own recommendation and correctly:** closing D18 while the manifest still describes hash-pinning as forthcoming would retire the ticket while leaving a document that claims a property the code lacks — D18's exact shape, one level up. Real fix needs a build step this machine cannot host (no container runtime), so this is genuinely blocked on infrastructure, not effort. | 🟠 High — bounds what the sandbox can be claimed to protect against |
