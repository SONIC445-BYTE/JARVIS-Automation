...
| **D15** | **DEC-002's audit trail does not cover two live handlers.** `jarvis.py`'s dispatch chain checks `intent.handler == "code_engine"` (line 883) and `elif intent.handler == "rhinal_capture"` (line 895) and calls each subsystem **directly**, never constructing a `daemon.intent_parser.Intent` and therefore never reaching `UIExecutor.execute_intent()` — so neither emits a who/what/when/why/source/consent record. DEC-002's own claim ("the single pre-adapter chokepoint every `Intent` passes through") is literally true and still misleading: these two handlers never *become* `Intent`s. **`rhinal_capture` is the one that matters** — it performs a real write to the physician's external Rhinal vault, unaudited. `code_engine` writes code to disk, also unaudited. Found by an independent design review of Stage 0.5, then confirmed directly against `jarvis.py`. Not fixed on discovery — it revises a prior closure claim, which is a declared hard stop. Likely fix is routing both through the same chokepoint (or emitting explicitly from each branch), but "should a non-GUI MCP handler become an `Intent`?" is a real design question, not a mechanical swap. | 🟠 High — does not reopen DEC-002's architecture, but narrows what its coverage claim can honestly assert |
