...
- `jarvis_orb.py` — ~~built and **approved**, untested on Windows terminal, not yet wired~~. **Corrected v3.16, re-checked and found stale before acting on it, per instruction:** this file does not exist anywhere in the repository (any branch), any sibling checkout on this machine, or common outside-git locations (`Downloads/`, `OneDrive/`) — confirmed by `git log --all` and filesystem search, not assumed from this line's own prior claim. "Built and approved" describes a state this document asserted but that was never actually committed anywhere accessible. Not rebuilt here — the v3.16 wiring instruction was declined for exactly this reason (see execution log): wiring requires the module to exist, and reconstructing it from this note's description would be new design work, not wiring. See §3.8.
