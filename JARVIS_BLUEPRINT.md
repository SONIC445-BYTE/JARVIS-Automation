...
| **D18** | **The Level6 "sandbox" is not a sandbox, and one of its variables reads as a control it does not implement.** `AgentCore/level6/sandbox_runner.py:80–83` runs `subprocess.run([sys.executable, "-m", "pytest"], cwd=sandbox_dir)` — the **host** interpreter against the **host's** site-packages, with only a temp working directory for isolation. There is no container, no separate environment, and no egress restriction. Worse, line 65 sets `env["JARVIS_SANDBOX_NETWORK"] = "0"`, and a repo-wide grep finds that name **in exactly one place — where it is written.** Nothing reads it. It is decoration that reads like a network control, which is the same failure shape as D1 (`_fallback_listen` in a file named `local_stt.py`) and the `vaultWorthy` misread, and it is the precise thing this project's standing rule "never infer a contract from an identifier name" exists to catch — this time in our own code, about a security boundary. Found by the sandbox-design agent while resolving the package-access question, independently confirmed against the source before logging. **Consequence for §4.4b:** its Concrete gating requirements table describes controls the current runner cannot enforce; those rows are a specification for the image that must be built, not a description of today. Not fixed here — building real isolation is its own phase, and §4.4b's own position is that the ResolutionGate is the primary control with the sandbox as defence-in-depth, so this weakens a second layer rather than the first. | 🟠 High — nothing in Adaptive Adapter Generation may be built against the current runner while it is mistaken for a boundary |
...
| Network egress | **None — pre-staged dependencies, not a proxy** *(resolved; see finding 2 above)*. Fixed vetted set in `AgentCore/policy/adapter_sandbox_dependencies.yaml`: digest-pinned base, hashed lockfile over the full transitive closure, no index, no `pip` at runtime. An unvetted import fails closed *before execution* with a diagnostic naming the package, the import site, and the Boundary Ledger entries that triggered the generation — no auto-install, no network retry, no substituting a similar package, no silently rewriting the import. Outside-the-set packages are a reviewed-once developer escalation, expiring by default. **Not a capability restriction, and must not be read as one:** `socket`, `subprocess`, and `urllib.request` are standard library and cannot be curated away. This removes the fetch path and the supply-chain surface, not what generated code can attempt. |
